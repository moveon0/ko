 ...
 * 
 * -어댑터를 이용한 이벤트 처리
 * 리스너 인터페이스를 구현하기 위해서는 반드시 재정의해야한다.
 * 만약에 MouseListener를 구현해야 하는 경우, 다 사용하지 않더라도 5개 메서드를 모두 재정의해야한다.
 * 
 * 이러한 문제를 해결해주는 방법이 어댑터클래스(Adapter class)이다.
 * 어댑터 클래스는 리스너가 가지고 있는 메서드 중에서 필요한 메서드만 재정의해주면 된다.
 * 
